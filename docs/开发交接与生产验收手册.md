...
- 上传真实 PDF 后必须出现 `开始检查报告` 主按钮；点击后应进入运行态并显示进度/日志。2026-06-21 已用 Playwright 验证桌面宽屏和 390px 移动端无 console error。
- 最终 E2E 必须配置真实 DeepSeek，上传真实 PDF 并下载四种格式。

## 13. 自动化测试

2026-06-21 当前基线：`403 passed, 3 skipped`。新增覆盖空格分列表格、拆分表头、跨页表头、宽表日期组、日期感知来源行选择、单初始列、X/Y 初始列、力值测值列、原始候选 Excel 和来源质量门禁。
...
# 当前仓库没有独立 verify_msi.ps1；发布验收使用 msiexec 静默安装、
# 启动安装后的 EXE、再 msiexec 静默卸载，日志写入 output/。
```
