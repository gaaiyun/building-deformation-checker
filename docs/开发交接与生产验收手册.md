...
- 2026-06-28 当前源码已重建并验证：`pytest` 为 `409 passed, 3 skipped`；Streamlit Playwright 上传/点击 smoke 通过；EXE 直接启动 10 秒未退出；MSI 静默安装、安装后启动、静默卸载通过。
- 2026-06-28 真实模型 smoke：DeepSeek `deepseek-v4-flash` 质安错误/正确成对样本默认无缓存通过，正确版 0 error、错误版检出 12 error；PaddleOCR-VL-1.6 强制 OCR smoke 走 `paddle_ocr/primary` 成功。
- 最新本地证据目录：`output\actual_acceptance_20260628_092402`（质安成对样本）和 `output\actual_acceptance_20260628_092750`（Paddle 强制 OCR smoke）。这些目录被 `.gitignore` 排除，不提交 Git。
- 2026-06-28 产物 hash：EXE SHA256 `BE8D6FA86FB3B6C32772572873F046E600377505E20CDD01EB3F02AD20CE1E54`；MSI SHA256 `6C6E698181BE14160A1141EC56EA0801ACB516B3E712CFEF3C45C3EF306C7677`。
...
2026-06-28 当前基线：`409 passed, 3 skipped`。新增覆盖空格分列表格、拆分表头、跨页表头、宽表日期组、日期感知来源行选择、单初始列、X/Y 初始列、力值测值列、原始候选 Excel、来源质量门禁、流式响应/客户端资源关闭、元数据日期过滤、负数来源文本保真和力值绝对值惯例。
...
- 可用网络下 PaddleOCR-VL-1.6 提交、轮询和结果下载；2026-06-28 smoke 已通过，复杂原生 PDF 仍要补表格级效果。
