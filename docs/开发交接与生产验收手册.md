...
- GitHub Release：`https://github.com/gaaiyun/building-deformation-checker/releases/tag/v2.1.11`；已上传 EXE、编号 MSI、稳定名 MSI 和 `SHA256SUMS.txt`。
...
- `dist/` 本地产物不提交 Git 历史；需要交付给甲方时优先使用 GitHub Release `v2.1.11`，或使用本地 `dist/BuildingDeformationChecker.exe`、`dist/BuildingDeformationChecker-2.1.11.msi`、稳定名 `dist/BuildingDeformationChecker.msi`，并在交付记录里附 SHA256。
