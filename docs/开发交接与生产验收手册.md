...
- 当前本机发布产物：`dist\BuildingDeformationChecker.exe`、`dist\BuildingDeformationChecker-2.1.10.msi`、`dist\BuildingDeformationChecker.msi`。
- 2026-06-22 当前源码已重建并验证：`pytest` 为 `409 passed, 3 skipped`；Streamlit Playwright 上传/点击 smoke 通过；EXE 直接启动 10 秒未退出；MSI 静默安装、安装后启动、静默卸载通过。
- 2026-06-22 产物 hash：EXE SHA256 `CEE34DE5F9EE3DE5F3DDDFC8EB10D1570A6F59221E1A76860C69C78E780B9FAF`；MSI SHA256 `6B421F9ABF8A46C66EAA37B910B4B24DE0514F4364D130753C2FD40496364015`。
...
- 上传真实 PDF 后必须出现 `开始检查报告` 主按钮；点击后应进入运行态并显示进度/日志。2026-06-22 已用 Playwright 验证桌面宽屏和 390px 移动端无 console error，截图证据在 `G:\dev-cache\bdc-ui-qa\`（不入 Git）。
- 最终 E2E 必须配置真实 DeepSeek，上传真实 PDF 并下载四种格式。

## 13. 自动化测试

2026-06-22 当前基线：`409 passed, 3 skipped`。新增覆盖空格分列表格、拆分表头、跨页表头、宽表日期组、日期感知来源行选择、单初始列、X/Y 初始列、力值测值列、原始候选 Excel、来源质量门禁、流式响应/客户端资源关闭、元数据日期过滤、负数来源文本保真和力值绝对值惯例。
...
- MSI 静默安装、安装后启动、静默卸载。
- Streamlit 浏览器真实上传、主按钮出现、点击进入运行态；外部服务稳定后再补完整完成态四格式下载。
- 真实 DeepSeek 正确版和错误版样本对照。
- 可用网络下 PaddleOCR-VL-1.6 提交、轮询和结果下载。
...
- PaddleOCR 若在 HTTP 前被远端重置（Windows 10054），不能声称 OCR 已通过。
- EXE/MSI 未使用商业 Authenticode 证书签名，Windows 可能显示未知发布者。
- `dist/` 本地产物不提交 GitHub；需要交付给甲方时使用本地 `dist/BuildingDeformationChecker.exe`、`dist/BuildingDeformationChecker-2.1.10.msi` 或稳定名 `dist/BuildingDeformationChecker.msi`，并在交付记录里附 SHA256。
